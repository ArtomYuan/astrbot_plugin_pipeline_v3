...
判定规则（按优先级）：
1. 消息中直接称呼或提及某角色（含昵称）→ 优先判定为该角色
2. 用户问话对象明确（如「小庄，今天天气怎么样」）→ 该角色回应
3. 消息涉及多个角色/全体 → 列出所有相关角色
4. 只有场景/动作/环境描述、无角色互动 → 旁白
5. 与所有角色无关（闲聊/数学题/无关话题）→ []

示例：
「小庄，今天天气怎么样」→ ["庄方宜"]
「小陈，你在做什么」→ ["陈千语"]
「大家今天有什么安排」→ ["庄方宜","陈千语","佩丽卡"]
「我推开咖啡馆的门，走到靠窗的位置坐下」→ ["旁白"]
「1+1等于几」→ []
